...
const char *test_export()
{
	...
	tokenlist = tokenizer('export VAR1=string VAR2="string" VAR3= VAR4="" VAR5=2=3 VAR6');
	status = execute_export_builtin(env_arr, tokenlist);

	debug("VAR1 >>>>>>>>>>>>>>> -%s-", mini_get_env(env_arr, "VAR1"));
	my_assert(ft_strncmp(mini_get_env(env_arr, "VAR1"), "string", 7) == 0, "'export VAR1=string' did not correctly add 'string' string to VAR1");
	debug("VAR2 >>>>>>>>>>>>>>> -%s-", mini_get_env(env_arr, "VAR2"));
	my_assert(ft_strncmp(mini_get_env(env_arr, "VAR2"), "string", 7) == 0, "export VAR2=\"string\" did not correctly add 'string' string to VAR2");
	...
	my_assert(ft_strncmp(mini_get_env(env_arr, "VAR4"), "", 1) == 0, "export VAR4=\"\" did not add empty string value to VAR4");
	debug("VAR5 >>>>>>>>>>>>>>> -%s-", mini_get_env(env_arr, "VAR5"));
	my_assert(ft_strncmp(mini_get_env(env_arr, "VAR5"), "2=3", 4) == 0, "'export VAR5=2=3' did not correctly add '2=3' string to VAR5");
